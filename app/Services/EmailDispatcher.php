...
class EmailDispatcher{
    ...
    public static function sendPendingEnrolmentEmails(){
        ...
        $count = 0;
        $pendingMails = PendingEmails::where('status',0)->where('tries', '<=', 3)->limit(100);//->get();
    ...
    public static function sendMail($mail_subject, $mail_body, $recipient){  
        ...
        $data = array("sender"=>"noreply@fidelitybank", "from"=> "noreply@fidelitybank.ng","to"=>$recipient, "bcc"=> "greenrewards@perxclm.com", "subject"=>$mail_subject, "body"=> $mail_body);
        $url = env('EMAIL_SERVICE_URL', 'https://loyaltysolutionsnigeria.com/email_templates/sendmail2.php');
         $mail_sent_response =   CurlService::doCURL($url, $data);
         //print_r($mail_sent_response);
